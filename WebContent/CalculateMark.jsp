...
<body class="cbp-spmenu-push">
  ...
  <div class="panel-heading"><h3> Marks Calculated </h3></div>
  <div class="panel-body">
		     ...
		     <%
		     float totalmarks=0;
		     String[] words = request.getParameterValues("score");
		     
		  for(int k1=0;k1<words.length;k1++){
			  
			  float f1=Float.parseFloat(words[k1]);
			  totalmarks=totalmarks+f1;
		  }
		     
		     
		     %>
<h2><%out.println("Total Marks Obtain : "+totalmarks); %></h2>
<hr>
<h2><%out.println("Passing Marks : "+passingark); %></h2>
<hr>
<h2><%
float passing=Float.parseFloat(""+passingark);
String passingstatus="";
if(totalmarks>=passing){
out.println("Result : Candidate Pass");
passingstatus="Pass";
}else{
	out.println("Result : Candidate Fail");
passingstatus="Fail";
}
...
PreparedStatement prstm56=con.prepareStatement("select * from studentdata where studid=?");
prstm56.setString(1, studid);
ResultSet rs=prstm56.executeQuery();
String email="";
String studname="";
String subject="Test Result";
String body=" Your obtain "+totalmarks+"";
while(rs.next()){
	email=rs.getString(6).toString();
	studname=rs.getString(2).toString();
}
//out.println("Student"+studid);


PreparedStatement prstm89=con.prepareStatement("insert into results(studname,markdobtain,status,testid)values(?,?,?,?)");
prstm89.setString(1,studname);   
prstm89.setString(2, ""+totalmarks);
prstm89.setString(3,passingstatus);
prstm89.setString(4,request.getParameter("testid").trim());
prstm89.executeUpdate();

%>
<!-- 
<a href="Mail.jsp?to=<%out.println(email);%>&subject=<%out.println(subject);%>&body=<%out.println(body);%>"><input type="submit" value="Send Result By Mail"></A>
 -->
</div>
